                ...
                <div id="panelsStayOpen-collapseOne" className="accordion-collapse collapse show" aria-labelledby="headingOne">
                  <div className="accordion-body">
                    <ul className="list-unstyled">
                      {category&&category.map((item:Collection, id:number)=>
                      <li onClick={()=>handleChangeCategory(item)} key={id}><a href="#"  className={search.category.name===item.name?'text-dark':''} >{item.title} </a></li>
                      )}
                    </ul>
                  </div>
                </div>
                    ...
                    <div className="row mb-3">
                      <div className="col-6">
                        ...
                        <div className="form-outline">
                          <input type="number" value={search.min}  onFocus={(e)=>e.target.select()} className="form-control" name='min' onChange={handleAmount} />
                        </div>
                      </div>
                      <div className="col-6">
                        <p className="mb-0">
                        ...
                        <div className="form-outline">
                          <input type="number" name="max" value={search.max}  onFocus={(e)=>e.target.select()}  onChange={handleAmount} className="form-control" />
                    
                        </div>
                      </div>
                    </div>
                    <button type="button" onClick={handleFilter} className="btn btn-white w-100 border border-secondary">apply</button>
                  </div>
                </div>
              </div>
                ...
                <div id="panelsStayOpen-collapseFive" className="accordion-collapse collapse show" aria-labelledby="headingThree">
                  <div className="accordion-body">
                    ...
                    <div className="form-check">
                      <input className="form-check-input" type="checkbox" onChange={handleRating} value={search.rate5} name="rate5" checked ={search.rate5==='true'?true:false} />
                    ...
                    <input className="form-check-input" type="checkbox"  onChange={handleRating} value={search.rate4} name="rate4" checked ={search.rate4==='true'?true:false} />
                      <label className="form-check-label" htmlFor="flexCheckDefault">
                        <i className="fas fa-star text-warning"></i><i className="fas fa-star text-warning"></i><i className="fas fa-star text-warning"></i><i className="fas fa-star text-warning"></i>
                    ...
                    <input className="form-check-input" type="checkbox"  onChange={handleRating} value={search.rate3} name="rate3" checked ={search.rate3==='true'?true:false} />
                      <label className="form-check-label" htmlFor="flexCheckDefault">
                    ...
                    <input className="form-check-input" type="checkbox"  onChange={handleRating} value={search.rate2} name="rate2" checked ={search.rate2==='true'?true:false} />
...
{/* Main content start here */}
        ...
        <i className="fas fa-times-circle me-3" data-mdb-dismiss="alert" ></i>{errors.error_message}
      </div>:''}


          <header className="d-sm-flex align-items-center border-bottom mb-4 pb-3">
            <strong className="d-block py-2">{ filterContent.length} Items found </strong>
            <div className="ms-auto">

              <input className='form-control form-control-lg' onChange={handleSearch} value={search.filterText} name='filterText' placeholder='Type to search...'  />
...
{loader?<Loader />:filterContent&& dataList.currentData().map((item:Item, index:number)=>
              ...
              <div className="card w-100 my-2 shadow-2-strong">
                ...
                <div className="card-body d-flex flex-column">
                  <div className="d-flex flex-row">
                    <h5 className="mb-1 me-1 text-danger">{'0.132 ETH'}</h5>
                  </div>
                  <p className="card-text">{item.name}</p>
                  <div className="card-footer d-flex align-items-end pt-3 px-0 pb-0 mt-auto">

                    <Link className="btn btn-primary shadow-0 me-1 btn-block " to={`/assets/${item.contract}/${item.identifier}`}>
                    <a  >BUY NOW</a>
                    </Link>
                  </div>
                </div>
...
<>
      ...
      <hr />
        ...
        <li key={index} className={page===item?' page-item  active':'page-item '}><a className="page-link" href="#!" onClick={(e) =>handleChange(item)}>{item}</a></li>    
             )}
